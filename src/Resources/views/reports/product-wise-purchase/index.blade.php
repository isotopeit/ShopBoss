...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="#">
                <div class="row bg-light p-3">
                    ...
                    <div class="col-md mb-1 mt-5">
                        ...
                        @if (count($data) > 0)
                            <a href="/product-wise-purchase-report?from={{ $from }}&to={{ $to }}&product_id={{ implode(',',$product_id) }}&submit=print" title="{{ __('shopboss::shopboss.printData') }}" class="btn btn-primary fw-bold mt-3"
                                target="_blanck">
            ...
            @if (count($data) > 0)
                @include('shopboss::reports.product-wise-purchase.plan')
            @else
